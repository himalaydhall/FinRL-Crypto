...
def add_samples_equify_array_length(return_arrays):
    # Handle case where return_arrays is a list of arrays with different lengths
    if isinstance(return_arrays, list):
        no_arrays = len(return_arrays)
    else:
    ...
    for row in return_arrays:
        ...
        row_array = np.array(row) if isinstance(row, list) else row
        length = row_array.shape[0]
    ...
    for idx, row in enumerate(return_arrays):
        # Convert to numpy array if it's a list
        row_array = np.array(row) if isinstance(row, list) else row
        while row_array.shape[0] != max_length:
            row_array = np.append(row_array, row_array[-1])
        new_ret_array[:, idx] = row_array
...
pickle_results = [
                  "res_2026-03-08__11_59_39_model_KCV_ppo_5m_4H_25k"
                  ]
S = 14
