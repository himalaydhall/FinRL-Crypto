...
def embargo(cv: BaseTimeSeriesCrossValidator, train_indices: np.ndarray,
  ...
  df0 = prices.inde---
    ...
    # Convert to pandas Timestamp for compatibility with Timedelta
    last_test_eval_time = pd.Timestamp(last_test_eval_time)
    # Use numpy comparison to avoid pandas type issues
    cutoff_time = last_test_eval_time + cv.embargo_td
    # Convert cutoff_time to numpy datetime64 for comparison
    cutoff_time_np = cutoff_time.to_datetime64()
    if isinstance(cv.pred_times, pd.Series):
        pred_times_array = cv.pred_times.values
    else:
        pred_times_array = cv.pred_times
    # Use object dtype to avoid type promotion issues
    mask = pred_times_array.astype('datetime64[s]') <= cutoff_time_np.astype('datetime64[s]')
    min_train_index = len(pred_times_array[mask])
