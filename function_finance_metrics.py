...
def plot_pdf(sharpe_list_drl, sharpe_hodl, name, if_range_hodl=False):
    ...
    # Convert to list if it's a single value
    if not isinstance(sharpe_list_drl, (list, tuple, np.ndarray)):
        sharpe_list_drl = [sharpe_list_drl]
    if not isinstance(sharpe_hodl, (list, tuple, np.ndarray)):
...
def write_metrics_to_results(name, file_path, drl_cumrets, drl_annual_ret, drl_annual_vol, drl_sharpe_rat, vol,
                             append_write):
    # Handle both list and pandas Series for cumulative returns
    if hasattr(drl_cumrets, 'iloc'):
        cum_return = drl_cumrets.iloc[-1]
    else:
        cum_return = drl_cumrets[-1]
    
    with open(file_path, append_write) as f:
        f.write('\n################################## ' + name + ' ####################################\n')
        f.write('Cumulative return:           ' + str(cum_return * 100) + '\n')
