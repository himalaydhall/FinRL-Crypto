...
def download_CVIX(trade_start_date, trade_end_date):
    ...
    # Check if CVIX data was successfully downloaded
    if CVOL_df.empty:
        print("CVIX data not available, using dummy data")
        # Return dummy data with proper name
        return pd.Series([1.0], name='close')
...
def load_and_process_data(TIMEFRAME, trade_start_date, trade_end_date):
    ...
    CVIX_df = pd.merge(time_array.to_series(name='time'), CVIX_df, left_index=True, right_index=True, how='left')
...
pickle_results = ["res_2026-03-08__11_59_39_model_KCV_ppo_5m_4H_25k"]
